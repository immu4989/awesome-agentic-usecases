...
from service_visit_readiness_coordinator.agent import MockBackend
from service_visit_readiness_coordinator.evaluate import evaluate, score_run
from service_visit_readiness_coordinator.tools import TOOL_SCHEMAS, ToolSession
from service_visit_readiness_coordinator.world import generate_scenarios


def test_scenarios_are_balanced_and_reproducible():
    first = generate_scenarios(n=32, seed=317)
    second = generate_scenarios(n=32, seed=317)
...
def test_authority_trap_forbids_protected_decision():
    ...
    assert scenario.contract().expected_terminal == 'service_dispatch_review'
