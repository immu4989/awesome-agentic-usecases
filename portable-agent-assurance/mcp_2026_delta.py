...
def _json_object(pairs: list[tuple[str, Any]]) -> dict[str, Any]:
    ...
    for key, value in pairs:
        if key in result:
            raise McpDeltaError("duplicate JSON object key")
        result[key] = value
    return result


def _json_constant(value: str) -> Any:
    raise McpDeltaError("non-standard JSON numeric constant")
...
def _command_adapter(command: str, timeout: float):
    if (
        ...
        raise McpDeltaError("adapter timeout must be finite and greater than 0, up to 300 seconds")
