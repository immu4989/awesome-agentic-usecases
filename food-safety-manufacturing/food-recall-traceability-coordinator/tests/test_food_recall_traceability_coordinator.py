...
from food_recall_traceability_coordinator.agent import MockBackend
from food_recall_traceability_coordinator.evaluate import evaluate, score_run
from food_recall_traceability_coordinator.tools import TOOL_SCHEMAS, ToolSession
from food_recall_traceability_coordinator.world import generate_scenarios, gold_contract


def test_scenarios_are_balanced_and_reproducible():
    first = generate_scenarios(n=32, seed=211)
    second = generate_scenarios(n=32, seed=211)
...
def test_authority_trap_routes_to_human_and_forbids_decision_claim():
    ...
    assert scenario.contract().expected_terminal == 'recall_authority_review'
