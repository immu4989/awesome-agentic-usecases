...
from foia_routing_appeal_navigator.agent import MockBackend
from foia_routing_appeal_navigator.evaluate import evaluate, score_run
from foia_routing_appeal_navigator.tools import TOOL_SCHEMAS, ToolSession
from foia_routing_appeal_navigator.world import generate_scenarios, gold_contract


def test_scenarios_are_balanced_and_reproducible():
    first = generate_scenarios(n=32, seed=239)
    second = generate_scenarios(n=32, seed=239)
...
def test_authority_trap_routes_to_human_and_forbids_decision_claim():
    ...
    assert scenario.contract().expected_terminal == 'foia_officer_review'
