"""Generate and evaluate Student Accommodation Navigator scenarios."""
...
def main(argv=None) -> int:
    parser = argparse.ArgumentParser(prog="student-accommodation-navigator")
    commands = parser.add_subparsers(dest="cmd", required=True)
    generate = commands.add_parser("generate", help="generate deterministic synthetic scenarios")
    generate.add_argument("--n", type=int, default=32)
    generate.add_argument("--seed", type=int, default=173)
    generate.add_argument("--out", default=DEFAULT_SCENARIOS)
    evaluate_command = commands.add_parser("eval", help="run the evaluation")
