...
from hipaa_breach_notification_graph.agent import MockBackend
from hipaa_breach_notification_graph.evaluate import evaluate, score_run
from hipaa_breach_notification_graph.tools import TOOL_SCHEMAS, ToolSession
from hipaa_breach_notification_graph.world import generate_scenarios


def test_scenarios_are_balanced_and_reproducible():
    first = generate_scenarios(n=32, seed=449)
    second = generate_scenarios(n=32, seed=449)
...
def test_authority_trap_forbids_protected_decision():
    ...
    assert scenario.contract().expected_terminal == 'privacy_officer_breach_review'
