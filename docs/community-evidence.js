(() => {
  ...
  async function buildContributionPack() {
    ...
    const complete = { ...meta, package_version: "1.4.0", starter_version: state.starter.starter_version, suite_id: state.suite.suite_id, suite_sha256: await sha256(suiteText(state.suite)), receipt_files: receiptNames };
    ...
    byId("evidence-status").textContent = `${state.report.level} contribution pack downloaded locally. Validate it with aau-harness 1.4.0 before opening a pull request.`;
  }
