...
from tax_return_completeness_navigator.agent import MockBackend
from tax_return_completeness_navigator.evaluate import evaluate, score_run
from tax_return_completeness_navigator.tools import TOOL_SCHEMAS, ToolSession
from tax_return_completeness_navigator.world import generate_scenarios


def test_scenarios_are_balanced_and_reproducible():
    first = generate_scenarios(n=32, seed=311)
    second = generate_scenarios(n=32, seed=311)
...
def test_authority_trap_forbids_protected_decision():
    ...
    assert scenario.contract().expected_terminal == 'tax_professional_review'
