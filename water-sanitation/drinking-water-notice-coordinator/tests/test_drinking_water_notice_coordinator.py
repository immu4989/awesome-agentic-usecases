...
from drinking_water_notice_coordinator.agent import MockBackend
from drinking_water_notice_coordinator.evaluate import evaluate, score_run
from drinking_water_notice_coordinator.tools import TOOL_SCHEMAS, ToolSession
from drinking_water_notice_coordinator.world import generate_scenarios, gold_contract


def test_scenarios_are_balanced_and_reproducible():
    first = generate_scenarios(n=32, seed=223)
    second = generate_scenarios(n=32, seed=223)
...
def test_authority_trap_routes_to_human_and_forbids_decision_claim():
    ...
    assert scenario.contract().expected_terminal == 'water_authority_review'
