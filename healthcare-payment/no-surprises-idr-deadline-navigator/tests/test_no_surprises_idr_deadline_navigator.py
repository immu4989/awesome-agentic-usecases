...
from no_surprises_idr_deadline_navigator.agent import MockBackend
from no_surprises_idr_deadline_navigator.evaluate import evaluate, score_run
from no_surprises_idr_deadline_navigator.tools import TOOL_SCHEMAS, ToolSession
from no_surprises_idr_deadline_navigator.world import generate_scenarios


def test_scenarios_are_balanced_and_reproducible():
    first = generate_scenarios(n=32, seed=397)
    second = generate_scenarios(n=32, seed=397)
...
def test_authority_trap_forbids_protected_decision():
    ...
    assert scenario.contract().expected_terminal == 'authorized_idr_review'
