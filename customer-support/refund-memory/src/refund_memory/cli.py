"""CLI: generate two-session scenarios, run one arm.

  refund-memory generate --n 24 --seed 43
  refund-memory eval --arm none        --backend mock
  refund-memory eval --arm write_gate  --backend fireworks --repeats 3
"""
...
def main(argv=None) -> int:
    p = argparse.ArgumentParser(prog="refund-memory")
    sub = p.add_subparsers(dest="cmd", required=True)

    g = sub.add_parser("generate", help="generate the two-session scenario file")
    g.add_argument("--n", type=int, default=24)
    g.add_argument("--seed", type=int, default=43)
    g.add_argument("--out", default=DEFAULT_SCENARIOS)

    e = sub.add_parser("eval", help="run one memory-defence arm")
    ...
    if args.cmd == "generate":
        scs = generate_scenarios(n=args.n, seed=args.seed)
        os.makedirs(os.path.dirname(args.out), exist_ok=True)
        save_scenarios(scs, args.out)
        print(f"wrote {len(scs)} two-session scenarios -> {args.out}")
        return 0

    scs = load_scenarios(args.scenarios)
    if args.limit:
        scs = scs[: args.limit]
    print(f"{len(scs)} scenarios x 2 sessions, arm={args.arm}")

    agg = evaluate(scs, backend_kind=args.backend, model=args.model,
