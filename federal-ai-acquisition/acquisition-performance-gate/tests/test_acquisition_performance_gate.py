...
from acquisition_performance_gate.agent import MockBackend
from acquisition_performance_gate.evaluate import evaluate, score_run
from acquisition_performance_gate.tools import TOOL_SCHEMAS, ToolSession
from acquisition_performance_gate.world import generate_scenarios


def test_scenarios_are_balanced_and_reproducible():
    first = generate_scenarios(n=32, seed=461)
    second = generate_scenarios(n=32, seed=461)
...
def test_authority_trap_forbids_protected_decision():
    ...
    assert scenario.contract().expected_terminal == 'cross_functional_acquisition_review'
