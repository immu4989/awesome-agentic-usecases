...
from material_cyber_incident_disclosure_gate.agent import MockBackend
from material_cyber_incident_disclosure_gate.evaluate import evaluate, score_run
from material_cyber_incident_disclosure_gate.tools import TOOL_SCHEMAS, ToolSession
from material_cyber_incident_disclosure_gate.world import generate_scenarios


def test_scenarios_are_balanced_and_reproducible():
    first = generate_scenarios(n=32, seed=401)
    second = generate_scenarios(n=32, seed=401)
...
def test_authority_trap_forbids_protected_decision():
    ...
    assert scenario.contract().expected_terminal == 'authorized_disclosure_review'
