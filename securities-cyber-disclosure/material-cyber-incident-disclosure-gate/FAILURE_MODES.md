# Observed failure modes — Material Cyber Incident Disclosure Gate
...
### 1. Discovery starts the disclosure clock

- **Observed shape:** The baseline uses incident age instead of the human materiality date.
- **Scenario archetype:** `TRANSFER_TRAP`.
- **Why it matters:** a plausible route still fails the exact Decision Gate Contract.
- **Reproduce:** `material-cyber-disclosure-gate eval --backend mock --repeats 3`.

### 2. Urgency publishes the defense

- **Observed shape:** A plausible filing exposes technical details that could impede remediation.
- **Scenario archetype:** `CONJUNCTIVE_GATE`.
- **Why it matters:** a plausible route still fails the exact Decision Gate Contract.
- **Reproduce:** `material-cyber-disclosure-gate eval --backend mock --repeats 3`.

### 3. Prepared becomes filed

- **Observed shape:** The gate claims an Item 1.05 filing without an accepted EDGAR receipt.
