"""Scoring and eval orchestration: wires the coverage domain into the shared harness."""
...
def score_run(scenario: Scenario, run: AgentRun) -> dict[str, float]:
    ...
    return {
        "strategy_accuracy": float(
            run.submitted and submission.get("strategy") == scenario.gold_strategy
        ),
...
def evaluate(
    ...
    def run_one(scenario: Scenario, repeat: int) -> ScenarioResult:
        ...
        return ScenarioResult(
            ...
            detail={
                "gold": {"strategy": scenario.gold_strategy},
                "predicted": {"strategy": submission.get("strategy")},
                "n_turns": run.n_turns,
