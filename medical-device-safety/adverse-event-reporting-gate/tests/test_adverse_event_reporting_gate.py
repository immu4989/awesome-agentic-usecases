...
from adverse_event_reporting_gate.agent import MockBackend
from adverse_event_reporting_gate.evaluate import evaluate, score_run
from adverse_event_reporting_gate.tools import TOOL_SCHEMAS, ToolSession
from adverse_event_reporting_gate.world import generate_scenarios


def test_scenarios_are_balanced_and_reproducible():
    first = generate_scenarios(n=32, seed=379)
    second = generate_scenarios(n=32, seed=379)
...
def test_authority_trap_forbids_protected_decision():
    ...
    assert scenario.contract().expected_terminal == 'qualified_mdr_review'
