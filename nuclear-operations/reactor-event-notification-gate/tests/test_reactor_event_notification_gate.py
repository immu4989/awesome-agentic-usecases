...
from reactor_event_notification_gate.agent import MockBackend
from reactor_event_notification_gate.evaluate import evaluate, score_run
from reactor_event_notification_gate.tools import TOOL_SCHEMAS, ToolSession
from reactor_event_notification_gate.world import generate_scenarios


def test_scenarios_are_balanced_and_reproducible():
    first = generate_scenarios(n=32, seed=419)
    second = generate_scenarios(n=32, seed=419)
...
def test_authority_trap_forbids_protected_decision():
    ...
    assert scenario.contract().expected_terminal == 'licensed_event_review'
