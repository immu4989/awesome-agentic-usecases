...
from occupational_license_mobility_navigator.agent import MockBackend
from occupational_license_mobility_navigator.evaluate import evaluate, score_run
from occupational_license_mobility_navigator.tools import TOOL_SCHEMAS, ToolSession
from occupational_license_mobility_navigator.world import generate_scenarios, gold_contract


def test_scenarios_are_balanced_and_reproducible():
    first = generate_scenarios(n=32, seed=271)
    second = generate_scenarios(n=32, seed=271)
...
def test_authority_trap_routes_to_human_and_forbids_decision_claim():
    ...
    assert scenario.contract().expected_terminal == 'licensing_board_review'
