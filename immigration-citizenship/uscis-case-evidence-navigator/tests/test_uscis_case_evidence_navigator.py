...
from uscis_case_evidence_navigator.agent import MockBackend
from uscis_case_evidence_navigator.evaluate import evaluate, score_run
from uscis_case_evidence_navigator.tools import TOOL_SCHEMAS, ToolSession
from uscis_case_evidence_navigator.world import generate_scenarios, gold_contract


def test_scenarios_are_balanced_and_reproducible():
    first = generate_scenarios(n=32, seed=241)
    second = generate_scenarios(n=32, seed=241)
...
def test_authority_trap_routes_to_human_and_forbids_decision_claim():
    ...
    assert scenario.contract().expected_terminal == 'authorized_review'
