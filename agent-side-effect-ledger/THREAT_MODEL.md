...
| Command names one adapter while evidence hashes a decoy argument or same-named file outside the workspace | Reviewers retain bytes that were never the declared executable entrypoint | Matrix 0.5 requires the artifact at `argv[0]` or a supported `argv[1]` interpreter target, replaces that token with the declared absolute path, records the position, compares before/after bytes, and carries the original bytes | Equal observations do not prove continuous immutability or runtime identity |
| Entrypoint stays fixed while imported local implementation changes | Passing evidence is attached to behavior implemented by different source bytes | Python targets transitively capture static local imports, preserve their bytes, reject obvious dynamic loading, expose unresolved names, and compare every captured file after the matrix run | Static syntax is not the interpreter, installed packages, unexercised dynamic behavior, environment, container, builder, or a complete runtime graph |
| Runtime policy or data changes while entrypoint and imports stay fixed | Tested behavior is attached to a release configured by different workspace bytes | Matrix 0.5 injects a byte-bound CPython startup observer, requires all 109 reference processes, digest-samples regular workspace reads before application opens, and Release Binding 0.4 holds on a changed observed digest | Python hooks are bypassable, sampling has a TOCTOU window, runtime-only bytes are deliberately not embedded, and outside-workspace access is excluded |
| Tested/deployed adapter substitution | A passing matrix is attached to different source bytes, local import materials, observed workspace inputs, or a different release label | Release Binding 0.4 copies and hashes all three entrypoints and static material sets, then binds digest-only observed-material snapshots with the exact AABOM, release, matrix manifest, tool, and operation | The unsigned pack does not identify or observe a live workload |
...
adapter commands; they do not constrain what those programs can do. The Python-level observer adds
visibility but is explicitly not containment. The release binder reads and copies declared adapter
files and static-local material sets, then re-hashes observed workspace paths without executing
them. Matrix and release packs deliberately stop short of the complete execution environment. Run only trusted,
staging-only adapters in an appropriate sandbox. Do not place credentials, personal data, protected records, production
targets, proprietary adapter code, private traces, controlled information, or classified
information in a public suite, adapter request, receipt, material set, or binding pack.
