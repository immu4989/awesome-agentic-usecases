...
Research checked on 2026-09-05. This profile addresses a narrow evidence-substitution problem:
testing one adapter snapshot and then associating the receipt with a different release, tool,
operation, resource scope, or adapter. It binds copied bytes; it does not establish what is actually
running.
...
3. **One tested relationship cannot cover a tool family.** Coverage is calculated on the exact
...
10. **A scope label is not object-attribute proof.** The semantic profile intentionally supports
