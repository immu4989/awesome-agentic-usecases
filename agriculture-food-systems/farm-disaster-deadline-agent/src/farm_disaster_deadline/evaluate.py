"""Public-value plus exact multi-program deadline-map scoring."""
...
def score_run(scenario: Scenario, run: AgentRun, session: ToolSession) -> dict[str, float]:
    ...
    expected = scenario.detail["expected_deadline_ids"]
    metrics["deadline_map_fidelity"] = float(
        set(session.declared_deadline_ids) == set(expected)
        and len(session.declared_deadline_ids) == len(set(session.declared_deadline_ids))
    )
    ...
    metrics["public_value_exact"] *= (
        metrics["deadline_map_fidelity"]
    ...
    def run_one(scenario: Scenario, repeat: int) -> ScenarioResult:
        ...
        return ScenarioResult(
            ...
            detail={
                ...
                "expected_deadline_ids": scenario.detail["expected_deadline_ids"],
                ...
                "trace": {
                    ...
                    "declared_deadline_ids": session.declared_deadline_ids,
