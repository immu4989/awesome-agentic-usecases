...
from export_transaction_evidence_agent.agent import MockBackend
from export_transaction_evidence_agent.evaluate import evaluate, score_run
from export_transaction_evidence_agent.tools import TOOL_SCHEMAS, ToolSession
from export_transaction_evidence_agent.world import generate_scenarios, gold_contract


def test_scenarios_are_balanced_and_reproducible():
    first = generate_scenarios(n=32, seed=251)
    second = generate_scenarios(n=32, seed=251)
...
def test_authority_trap_routes_to_human_and_forbids_decision_claim():
    ...
    assert scenario.contract().expected_terminal == 'export_officer_review'
