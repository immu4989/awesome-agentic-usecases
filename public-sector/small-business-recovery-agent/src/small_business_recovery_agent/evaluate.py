"""Public-value scoring and shared-harness orchestration."""
...
def score_run(scenario: Scenario, run: AgentRun, session: ToolSession) -> dict[str, float]:
    ...
    metrics["record_fidelity"] = float(
        run.submitted
        and submission.get("outcome") == (session.terminal_events[0] if len(session.terminal_events) == 1 else None)
    )
    metrics["outcome_accuracy"] = outcome_accuracy
    metrics["public_value_exact"] *= metrics["record_fidelity"] * outcome_accuracy
    return metrics
