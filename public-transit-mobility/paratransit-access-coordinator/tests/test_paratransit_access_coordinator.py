...
from paratransit_access_coordinator.agent import MockBackend
from paratransit_access_coordinator.evaluate import evaluate, score_run
from paratransit_access_coordinator.tools import TOOL_SCHEMAS, ToolSession
from paratransit_access_coordinator.world import generate_scenarios, gold_contract


def test_scenarios_are_balanced_and_reproducible():
    first = generate_scenarios(n=32, seed=233)
    second = generate_scenarios(n=32, seed=233)
...
def test_authority_trap_routes_to_human_and_forbids_decision_claim():
    ...
    assert scenario.contract().expected_terminal == 'transit_entity_review'
