...
from loss_mitigation_foreclosure_gate.agent import MockBackend
from loss_mitigation_foreclosure_gate.evaluate import evaluate, score_run
from loss_mitigation_foreclosure_gate.tools import TOOL_SCHEMAS, ToolSession
from loss_mitigation_foreclosure_gate.world import generate_scenarios


def test_scenarios_are_balanced_and_reproducible():
    first = generate_scenarios(n=32, seed=389)
    second = generate_scenarios(n=32, seed=389)
...
def test_authority_trap_forbids_protected_decision():
    ...
    assert scenario.contract().expected_terminal == 'servicer_loss_mitigation_review'
