...
from hazardous_waste_manifest_coordinator.agent import MockBackend
from hazardous_waste_manifest_coordinator.evaluate import evaluate, score_run
from hazardous_waste_manifest_coordinator.tools import TOOL_SCHEMAS, ToolSession
from hazardous_waste_manifest_coordinator.world import generate_scenarios


def test_scenarios_are_balanced_and_reproducible():
    first = generate_scenarios(n=32, seed=353)
    second = generate_scenarios(n=32, seed=353)
...
def test_authority_trap_forbids_protected_decision():
    ...
    assert scenario.contract().expected_terminal == 'environmental_compliance_review'
