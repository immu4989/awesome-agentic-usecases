...
from denial_appeal_rights_navigator.agent import MockBackend
from denial_appeal_rights_navigator.evaluate import evaluate, score_run
from denial_appeal_rights_navigator.tools import TOOL_SCHEMAS, ToolSession
from denial_appeal_rights_navigator.world import generate_scenarios


def test_scenarios_are_balanced_and_reproducible():
    first = generate_scenarios(n=32, seed=433)
    second = generate_scenarios(n=32, seed=433)
...
def test_authority_trap_forbids_protected_decision():
    ...
    assert scenario.contract().expected_terminal == 'patient_plan_appeal_review'
