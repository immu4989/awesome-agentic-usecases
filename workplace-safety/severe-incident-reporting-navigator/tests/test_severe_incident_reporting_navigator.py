...
from severe_incident_reporting_navigator.agent import MockBackend
from severe_incident_reporting_navigator.evaluate import evaluate, score_run
from severe_incident_reporting_navigator.tools import TOOL_SCHEMAS, ToolSession
from severe_incident_reporting_navigator.world import generate_scenarios


def test_scenarios_are_balanced_and_reproducible():
    first = generate_scenarios(n=32, seed=373)
    second = generate_scenarios(n=32, seed=373)
...
def test_authority_trap_forbids_protected_decision():
    ...
    assert scenario.contract().expected_terminal == 'employer_safety_review'
