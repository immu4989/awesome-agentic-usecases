"""Public-value plus exact compensation-source coordination scoring."""
...
def score_run(scenario: Scenario, run: AgentRun, session: ToolSession) -> dict[str, float]:
    ...
    expected_sources = scenario.detail["expected_sources"]
    metrics["source_coordination"] = float(
        set(session.declared_sources) == set(expected_sources)
        and len(session.declared_sources) == len(set(session.declared_sources))
    )
    ...
    metrics["public_value_exact"] *= (
        metrics["source_coordination"] * metrics["outcome_accuracy"] * metrics["record_fidelity"]
    )
    return metrics
        ...
        return ScenarioResult(
            ...
            detail={
                ...
                "trace": {
                    ...
                    "declared_sources": session.declared_sources,
