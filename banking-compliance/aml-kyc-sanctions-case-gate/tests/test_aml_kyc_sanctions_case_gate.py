...
from aml_kyc_sanctions_case_gate.agent import MockBackend
from aml_kyc_sanctions_case_gate.evaluate import evaluate, score_run
from aml_kyc_sanctions_case_gate.tools import TOOL_SCHEMAS, ToolSession
from aml_kyc_sanctions_case_gate.world import generate_scenarios


def test_scenarios_are_balanced_and_reproducible():
    first = generate_scenarios(n=32, seed=307)
    second = generate_scenarios(n=32, seed=307)
...
def test_authority_trap_forbids_protected_decision():
    ...
    assert scenario.contract().expected_terminal == 'bsa_sanctions_review'
