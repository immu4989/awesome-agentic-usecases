...
from aircraft_dispatch_evidence_gate.agent import MockBackend
from aircraft_dispatch_evidence_gate.evaluate import evaluate, score_run
from aircraft_dispatch_evidence_gate.tools import TOOL_SCHEMAS, ToolSession
from aircraft_dispatch_evidence_gate.world import generate_scenarios


def test_scenarios_are_balanced_and_reproducible():
    first = generate_scenarios(n=32, seed=293)
    second = generate_scenarios(n=32, seed=293)
...
def test_authority_trap_forbids_protected_decision():
    ...
    assert scenario.contract().expected_terminal == 'dispatcher_pic_review'
