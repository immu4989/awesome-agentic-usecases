...
from nursing_home_transfer_discharge_navigator.agent import MockBackend
from nursing_home_transfer_discharge_navigator.evaluate import evaluate, score_run
from nursing_home_transfer_discharge_navigator.tools import TOOL_SCHEMAS, ToolSession
from nursing_home_transfer_discharge_navigator.world import generate_scenarios


def test_scenarios_are_balanced_and_reproducible():
    first = generate_scenarios(n=32, seed=409)
    second = generate_scenarios(n=32, seed=409)
...
def test_authority_trap_forbids_protected_decision():
    ...
    assert scenario.contract().expected_terminal == 'resident_facility_rights_review'
