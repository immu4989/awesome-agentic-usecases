...
from provisional_ballot_status_navigator.agent import MockBackend
from provisional_ballot_status_navigator.evaluate import evaluate, score_run
from provisional_ballot_status_navigator.tools import TOOL_SCHEMAS, ToolSession
from provisional_ballot_status_navigator.world import generate_scenarios, gold_contract


def test_scenarios_are_balanced_and_reproducible():
    first = generate_scenarios(n=32, seed=263)
    second = generate_scenarios(n=32, seed=263)
...
def test_authority_trap_routes_to_human_and_forbids_decision_claim():
    ...
    assert scenario.contract().expected_terminal == 'election_official_review'
