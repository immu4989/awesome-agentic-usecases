...
from debt_validation_dispute_navigator.agent import MockBackend
from debt_validation_dispute_navigator.evaluate import evaluate, score_run
from debt_validation_dispute_navigator.tools import TOOL_SCHEMAS, ToolSession
from debt_validation_dispute_navigator.world import generate_scenarios


def test_scenarios_are_balanced_and_reproducible():
    first = generate_scenarios(n=32, seed=359)
    second = generate_scenarios(n=32, seed=359)
...
def test_authority_trap_forbids_protected_decision():
    ...
    assert scenario.contract().expected_terminal == 'consumer_or_counsel_review'
