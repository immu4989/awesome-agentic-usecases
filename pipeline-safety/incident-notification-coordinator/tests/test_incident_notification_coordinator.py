...
from incident_notification_coordinator.agent import MockBackend
from incident_notification_coordinator.evaluate import evaluate, score_run
from incident_notification_coordinator.tools import TOOL_SCHEMAS, ToolSession
from incident_notification_coordinator.world import generate_scenarios


def test_scenarios_are_balanced_and_reproducible():
    first = generate_scenarios(n=32, seed=443)
    second = generate_scenarios(n=32, seed=443)
...
def test_authority_trap_forbids_protected_decision():
    ...
    assert scenario.contract().expected_terminal == 'pipeline_incident_review'
