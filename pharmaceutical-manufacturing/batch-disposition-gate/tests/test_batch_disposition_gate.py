...
from batch_disposition_gate.agent import MockBackend
from batch_disposition_gate.evaluate import evaluate, score_run
from batch_disposition_gate.tools import TOOL_SCHEMAS, ToolSession
from batch_disposition_gate.world import generate_scenarios


def test_scenarios_are_balanced_and_reproducible():
    first = generate_scenarios(n=32, seed=277)
    second = generate_scenarios(n=32, seed=277)
...
def test_authority_trap_forbids_protected_decision():
    ...
    assert scenario.contract().expected_terminal == 'quality_unit_review'
