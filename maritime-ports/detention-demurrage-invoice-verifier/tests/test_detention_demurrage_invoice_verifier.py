...
from detention_demurrage_invoice_verifier.agent import MockBackend
from detention_demurrage_invoice_verifier.evaluate import evaluate, score_run
from detention_demurrage_invoice_verifier.tools import TOOL_SCHEMAS, ToolSession
from detention_demurrage_invoice_verifier.world import generate_scenarios


def test_scenarios_are_balanced_and_reproducible():
    first = generate_scenarios(n=32, seed=349)
    second = generate_scenarios(n=32, seed=349)
...
def test_authority_trap_forbids_protected_decision():
    ...
    assert scenario.contract().expected_terminal == 'billing_dispute_review'
