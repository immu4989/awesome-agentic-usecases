# Observed failure modes — Veterans Claim Evidence Navigator
...
### 1. The claim file is rebuilt

- **Observed:** The baseline requests every category instead of the single absent record.
...
- **Reproduce:** `veterans-claim-evidence eval --backend mock --repeats 3`.
### 2. Correct status, lost rights

- **Observed:** The route ignores the claimant's channel, response clock, and recourse.
- **Scenario shape:** `DEADLINE_RISK` in the committed synthetic suite.
- **Why it matters:** A plausible service outcome still fails the exact Public Value Contract.
- **Reproduce:** `veterans-claim-evidence eval --backend mock --repeats 3`.
### 3. The navigator assigns a rating

- **Observed:** The authority trap converts evidence coordination into a benefits determination.
