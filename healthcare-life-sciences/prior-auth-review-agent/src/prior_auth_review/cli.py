"""CLI for the review-record A/B.
  ...
  prior-auth-review-agent eval --arm none        --backend fireworks --repeats 3
  prior-auth-review-agent eval --arm prompt_guard --backend fireworks --repeats 3
  prior-auth-review-agent eval --arm record_gate  --backend fireworks --repeats 3
"""
...
from .world import ARMS, generate_requests, load_requests, save_requests
...
def main(argv=None) -> int:
    p = argparse.ArgumentParser(prog="prior-auth-review-agent")
    sub = p.add_subparsers(dest="cmd", required=True)

    g = sub.add_parser("generate", help="write the request set")
    g.add_argument("--n", type=int, default=28, help="requests (7 archetypes)")
    g.add_argument("--seed", type=int, default=41)
    ...
    if args.cmd == "generate":
        reqs = generate_requests(n_per_archetype=max(1, args.n // 7), seed=args.seed)
        os.makedirs(os.path.dirname(args.out), exist_ok=True)
        save_requests(reqs, args.out)
        print(f"wrote {len(reqs)} requests -> {args.out}")
        return 0

    reqs = (load_requests(args.scenarios) if os.path.exists(args.scenarios)
            else generate_requests())
    if args.limit:
        reqs = reqs[: args.limit]
    print(f"{len(reqs)} requests, arm={args.arm}")
    agg = evaluate(reqs, backend_kind=args.backend, model=args.model,
