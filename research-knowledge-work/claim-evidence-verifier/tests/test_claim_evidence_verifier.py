...
from claim_evidence_verifier.agent import MockBackend
from claim_evidence_verifier.evaluate import evaluate, score_run
from claim_evidence_verifier.tools import TOOL_SCHEMAS, ToolSession
from claim_evidence_verifier.world import generate_scenarios


def test_scenarios_are_balanced_and_reproducible():
    first = generate_scenarios(n=32, seed=313)
    second = generate_scenarios(n=32, seed=313)
...
def test_authority_trap_forbids_protected_decision():
    ...
    assert scenario.contract().expected_terminal == 'editorial_fact_check_review'
