...
from product_recall_remedy_coordinator.agent import MockBackend
from product_recall_remedy_coordinator.evaluate import evaluate, score_run
from product_recall_remedy_coordinator.tools import TOOL_SCHEMAS, ToolSession
from product_recall_remedy_coordinator.world import generate_scenarios


def test_scenarios_are_balanced_and_reproducible():
    first = generate_scenarios(n=32, seed=347)
    second = generate_scenarios(n=32, seed=347)
...
def test_authority_trap_forbids_protected_decision():
    ...
    assert scenario.contract().expected_terminal == 'recalling_firm_review'
