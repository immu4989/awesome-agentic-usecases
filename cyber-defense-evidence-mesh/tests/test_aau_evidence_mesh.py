...
def _fixture(tmp_path: Path) -> Path:
    ...
    for name, kind, version, field, summary, extra in rows:
        ...
        declarations.append({"artifact_id": f"reference-{name}", "kind": kind, "path": f"artifacts/{name}.json", "evidence_level": "synthetic_reference", "producer": "AAU test fixture", "independent_reproduction": False})
    contract = {
        "mesh_version": "aau-cyber-defense-evidence-mesh/0.1", "mesh_id": "test-mesh", "title": "Test evidence mesh", "producer": "AAU tests", "artifacts": declarations,
...
def test_parent_traversal_and_false_independence_are_rejected(tmp_path):
    ...
    with pytest.raises(MeshError, match="independent_reproduction=true"):
        validate_contract(contract)
