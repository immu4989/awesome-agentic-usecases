...
CONTRACT_VERSION = "aau-cyber-defense-evidence-mesh/0.1"
INDEX_VERSION = "aau-cyber-defense-evidence-index/0.1"
PACK_VERSION = "aau-cyber-defense-evidence-pack/0.1"
MAX_BYTES = 2_000_000
VERSIONS = {
...
def _exact(value: Any, keys: set[str], label: str) -> dict[str, Any]:
    if not isinstance(value, dict) or set(value) != keys:
        raise MeshError(f"{label} fields differ from the 0.1 contract")
    return value
...
def validate_contract(contract: dict[str, Any]) -> None:
    ...
    for index, item in enumerate(artifacts):
        _exact(item, {"artifact_id", "kind", "path", "evidence_level", "producer", "independent_reproduction"}, f"artifacts[{index}]")
        ...
        if not isinstance(item["independent_reproduction"], bool):
            raise MeshError("independent_reproduction must be boolean")
        if item["evidence_level"] == "independently_reproduced" and not item["independent_reproduction"]:
            raise MeshError("independently_reproduced requires independent_reproduction=true")
...
def build_index(contract_path: Path) -> dict[str, Any]:
    ...
    for declaration in contract["artifacts"]:
        ...
        records.append({
            ...
            "independent_reproduction": declaration["independent_reproduction"],
...
def verify_pack(pack: Path) -> None:
    ...
    if {item.get("path") for item in manifest["files"]} != expected:
        raise MeshError("manifest file set differs from the 0.1 pack")
