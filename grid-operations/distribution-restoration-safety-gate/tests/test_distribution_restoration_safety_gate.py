...
from distribution_restoration_safety_gate.agent import MockBackend
from distribution_restoration_safety_gate.evaluate import evaluate, score_run
from distribution_restoration_safety_gate.tools import TOOL_SCHEMAS, ToolSession
from distribution_restoration_safety_gate.world import generate_scenarios


def test_scenarios_are_balanced_and_reproducible():
    first = generate_scenarios(n=32, seed=281)
    second = generate_scenarios(n=32, seed=281)
...
def test_authority_trap_forbids_protected_decision():
    ...
    assert scenario.contract().expected_terminal == 'system_operator_review'
