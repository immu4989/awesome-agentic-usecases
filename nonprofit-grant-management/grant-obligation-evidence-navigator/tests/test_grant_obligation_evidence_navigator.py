...
from grant_obligation_evidence_navigator.agent import MockBackend
from grant_obligation_evidence_navigator.evaluate import evaluate, score_run
from grant_obligation_evidence_navigator.tools import TOOL_SCHEMAS, ToolSession
from grant_obligation_evidence_navigator.world import generate_scenarios


def test_scenarios_are_balanced_and_reproducible():
    first = generate_scenarios(n=32, seed=331)
    second = generate_scenarios(n=32, seed=331)
...
def test_authority_trap_forbids_protected_decision():
    ...
    assert scenario.contract().expected_terminal == 'grant_officer_review'
